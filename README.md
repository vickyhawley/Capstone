...
# 2. Link and deploy the API
cd apps/api
vercel link                       # project name: groundwork-api
vercel --prod                     # note the URL that comes back

# 3. Point the web project at the API URL
#    Edit apps/web/vercel.json — replace the placeholder host in the rewrite
#    destination with the URL from step 2.

# 4. Link and deploy the web shell
cd ../web
vercel link                       # project name: groundwork-web
vercel --prod
```

The web URL is the public URL for the project. Verify with:

```bash
curl https://<web-url>/api/health
```

Should return `{"status":"ok","service":"groundwork-api",...}`.
